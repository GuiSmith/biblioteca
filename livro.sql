...
-- Estado
CREATE TABLE biblioteca.estado_livro (
    id SERIAL PRIMARY KEY,                -- Identificador único para cada estado
    ativo BOOLEAN DEFAULT TRUE,            -- Se o estado está ativo ou não
    descricao VARCHAR(255) NOT NULL,       -- Descrição do estado (ex: 'disponível', 'emprestado', 'danificado', etc.)
    data_cadastro TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- Data de cadastro do estado
);
-- Entrada
CREATE TABLE biblioteca.entrada (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE exemplar (
    ...
    FOREIGN KEY (id_estado_livro) REFERENCES biblioteca.estado_livro(id),  -- Relaciona o exemplar com o estado do livro
    FOREIGN KEY (id_entrada) REFERENCES biblioteca.entrada(id)
);